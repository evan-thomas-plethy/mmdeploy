...
Default geometry matches mmpose val_pipeline (train geometry minus augs):
  GetBBoxCenterScale(padding=1.25) -> TopdownAffine(input_size=(192, 256))
  -> SimCC decode (argmax, / split_ratio) -> map coords back to image space

Written with plain arithmetic + one 3-point affine solve so the same steps
port to Swift / Java (see MOBILE PORT notes under each function).

Legacy letterbox helpers (iOS preprocessImageRTMPose) remain at the bottom for
production-pipeline experiments; val export scripts do not use them.
"""
...
ASPECT_RATIO = TARGET_WIDTH / TARGET_HEIGHT  # w/h = 192/256 = 0.75
NUM_KEYPOINTS = 17
KEYPOINT_SCORE_THR = 0.2
NMS_THR = 0.9
BBOX_PADDING = 1.25  # GetBBoxCenterScale default
SIMCC_SPLIT_RATIO = 2.0  # codec.simcc_split_ratio
...
# ---------------------------------------------------------------------------
# Top-down affine preprocessing (DEFAULT for exported-model val).
# Step-for-step port of mmpose GetBBoxCenterScale + TopdownAffine
# (use_udp=False, rotation=0 at inference).
#
# Pipeline (same logical steps as mmpose val_pipeline):
#   1. bbox_xyxy_to_center_scale  — GetBBoxCenterScale
#   2. fix_aspect_ratio           — TopdownAffine._fix_aspect_ratio
#   3. get_warp_matrix            — mmpose get_warp_matrix (3 point pairs)
#   4. warpAffine bilinear        — TopdownAffine warp to (192, 256)
#   5. decode_simcc               — get_simcc_maximum + / split_ratio
#   6. model_coords_to_image      — TopdownPoseEstimator.add_pred_to_datasample
# ---------------------------------------------------------------------------
...
def bbox_xyxy_to_center_scale(bbox_xyxy, padding=BBOX_PADDING):
    ...
    scale  = bbox (w, h) * padding   # padding=1.25 by default

    MOBILE PORT:
      cx = (x1 + x2) * 0.5; cy = (y1 + y2) * 0.5
      sw = (x2 - x1) * padding; sh = (y2 - y1) * padding
    """
...
def fix_aspect_ratio(scale, aspect_ratio=ASPECT_RATIO):
    """Grow scale (w, h) so w/h == aspect_ratio. Mirrors _fix_aspect_ratio.

    if w > h * aspect: h = w / aspect
    else:              w = h * aspect

    MOBILE PORT: identical if/else on floats; no allocation needed.
    """
...
def solve_affine_from_correspondences(src, dst):
    """Solve 2x3 affine M such that dst ~= M @ [src; 1] for 3 point pairs.

    Pure linear algebra (no OpenCV). Same math every mobile platform needs:
      For each i:  x'_i = a*x_i + b*y_i + c
                   y'_i = d*x_i + e*y_i + f
      Stack into 6x6 system and solve for [a,b,c,d,e,f].

    MOBILE PORT:
      - Android: Matrix.setPolyToPoly(srcFlat, 0, dstFlat, 0, 3)
      - iOS:     same 6x6 solve, or vImage / Core Image affine
      - Java:    Apache Commons Math / EJML, or hand-solve 6x6
    """
    src = np.asarray(src, dtype=np.float64)
    dst = np.asarray(dst, dtype=np.float64)
    assert src.shape == (3, 2) and dst.shape == (3, 2)

    # Build A x = b for the 6 unknowns.
    ...
    return params.reshape(2, 3).astype(np.float64)
...
def get_warp_matrix(center, scale, rot_deg, output_size=INPUT_SIZE):
    ...
    Faithful reimplementation of mmpose get_warp_matrix (use_udp=False):
    build three source/destination point correspondences, then solve affine.

    MOBILE PORT: build the SAME 3 src/dst points, then call
    solve_affine_from_correspondences (or platform setPolyToPoly).
    """
    ...
    return solve_affine_from_correspondences(src, dst)
...
def topdown_affine(image_rgb, center, scale, output_size=INPUT_SIZE):
    ...
    MOBILE PORT:
      - Android: Bitmap + Matrix (setPolyToPoly) + Canvas.drawBitmap
      - iOS:     CIAffineTransform / vImagePiecewiseWarp_ARGB8888
      Border = black (0); interpolation = bilinear.
    """
...
def preprocess_topdown(image_rgb, bbox_xyxy, output_size=INPUT_SIZE,
    ...
    Steps mirror mmpose val:
      center, scale = GetBBoxCenterScale(bbox, padding)
      scale = fix_aspect_ratio(scale, w/h)
      warped = TopdownAffine(img, center, scale)   # fills (192, 256)
...
def decode_simcc(simcc_x, simcc_y, split_ratio=SIMCC_SPLIT_RATIO):
    ...
    MOBILE PORT: per-keypoint argmax over the two 1-D vectors; divide by 2.0.
    """
...
def model_coords_to_image(coords_model, center, scale, output_size=INPUT_SIZE):
    ...
    MOBILE PORT (per keypoint):
      x = kx / 192 * scaleW + centerX - 0.5 * scaleW
      y = ky / 256 * scaleH + centerY - 0.5 * scaleH
    """
...
# ---------------------------------------------------------------------------
# CoreML I/O helpers + COCO NMS / formatting (shared by val export scripts).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Legacy letterbox geometry (iOS preprocessImageRTMPose / applyDetectionCrop).
# Kept for production-pipeline experiments; NOT used by val export scripts.
# ---------------------------------------------------------------------------
...
def preprocess_letterbox(image_rgb, output_size=INPUT_SIZE):
    ...
    Matches iOS preprocessImageRTMPose. Prefer preprocess_topdown for val / training
    geometry parity.
    """
...
def preprocess_rtmpose_mobile(image_rgb, bbox_xyxy=None, output_size=INPUT_SIZE,
    ...
    """Legacy mobile preprocess: optional 1.25x crop then letterbox."""
